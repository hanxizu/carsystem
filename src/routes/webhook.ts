...
app.use(express.text({ type: 'text/xml' })); // 兼容XML（如果后面切加密模式）
  ...
  try {
    ...
    if (body.msgtype === 'text') {
      ...
      // 👉 选择险种
        ...
        await weworkService.updateCustomerRemark(
          userId,
          `${session.vehicleInfo!.plate}_${session.vehicleInfo!.ownerName}`
        );
      ...
      if (session && session.step === 'uploading') {
        ...
        if (session.businessDirection === 'renewal') {
          if (!session.uploadedFiles.drivingLicense) {
            ...
            const vehicleInfo =
              await ocrService.recognizeDrivingLicense(imageBuffer);
    ...
    return res.json({ success: true }); // ⚠️ 必须返回 success
  }
});

// ================== 报价回调 ==================
app.post('/quote-callback', async (req, res) => {
  const body = req.body;
  ...
  try {
    if (body.msgtype === 'image') {
      ...
      if (quoteData.displayId) {
        ...
        const quotes = await taskService.getQuotesByTask(
          quoteData.displayId
        );
        ...
        if (quotes.length >= 3) {
          ...
          if (task) {
            const summary = quotes
              .map(
                (q) => `${q.channel}: ${q.combo} - ${q.premium}元`
              )
              .join('\n');
    ...
    console.error('quote callback error:', err);
    return res.json({ success: true });
  }
});
